...
def run_torch_model(model, x_torch, iterations, warm_ups):
    start_events = [torch.cuda.Event(enable_timing=True) for _ in range(iterations)]
    end_events = [torch.cuda.Event(enable_timing=True) for _ in range(iterations)]
    # model = torch.compile(model)
    # Warmup steps
    for _ in range(warm_ups):
        model(x_torch) # don't record time
...
def run_sequence_graph(symbolic_traced, x1, iterations, warm_ups, start_index, end_index):
    with torch.no_grad():
        for i in range(warm_ups):
            symbolic_traced(x1)
            # my_interpreter.run(x1)

    with torch.no_grad():
        g1 = torch.cuda.CUDAGraph()

        with torch.cuda.graph(g1):
            out_torch_graph_without_stream = symbolic_traced(x1)
...
if __name__ == '__main__':
    ...
    y = run_torch_model(model, x, iterations, warm_ups)

    run_sequence_graph(model, x, iterations, warm_ups, 0, 300)


    Opara = GraphCapturer.capturer((x,), model)
    output = run_parallel_graph(Opara, (x,), iterations, warm_ups, 0, 300)

    print("output of PyTorch == output of Opara:", torch.allclose(y, output[0],rtol=1e-07,atol=1e-07,equal_nan =False), end='     ')
    print('Absolute difference:', torch.max(torch.abs(y.detach() - output[0].detach())))